...
const Lawyer = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const who = 'Lawyer';
      const response = await firebase.createUser(data, who);
      if (response && response.data) {
        console.log(response.data); // Ensure that the response has the expected data
        navigate('/approval');
      } else {
        console.error('User creation failed, no data found');
        setError('name', { type: 'manual', message: 'User creation failed. Please try again.' });
      }
    } catch (error) {
  ...
  const handleSignIn = async () => {
    try {
      ...
      if (user && user.data) {
        console.log(user.data); // Access the `data` field safely
        setEmail('');
        setPassword('');
        navigate('/approval');
      } else {
        console.error('Sign-in failed, no user data found');
        setError('email', { type: 'manual', message: 'Sign-in failed. Please check your credentials.' });
      }
    } catch (error) {
    ...
    <>
      <div className='flex justify-center'>
        <div className='flex flex-col'>
          <h2 className='text-center text-xl mt-6 border-b border-gray-600 p-2'>
            Lawyer Registration or <span className='text-blue-600 underline cursor-pointer' onClick={() => setLogin(!login)}>login</span>
          </h2>
          <form onSubmit={handleSubmit(onSubmit)} className='mt-5 flex'>
            {step === 1 && (
              <div className='flex flex-col'>
                ...
                <input
                  ...
                  className='bg-[#161616] p-1 focus:outline-none rounded-sm m-2 w-[20rem]'
                />
                <button type="button" onClick={nextStep} className='bg-blue-600 p-1 rounded-md mt-1 hover:bg-blue-600'>
                  Next
                </button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className='flex flex-col'>
                ...
                <input
                  ...
                  {...register('barAssociation', { required: 'Bar Association is required' })}
            ...
            {step === 3 && (
              <div className='flex flex-col'>
                ...
                <input
                  type="file"
                  {...register('certificates', { required: 'Certificate is required' })}
              ...
              <div className='w-[30rem] h-[20rem] rounded-lg'>
                ...
                <div>
                  <div className='pl-6 pr-6'>
                    ...
                    <button className='p-1 bg-blue-400 w-full rounded-sm mt-3' onClick={handleSignIn}>
                      Sign In
                    </button>
                  </div>
                </div>
